...
class App extends Component {
  ...
  convertThaiSystem({ rai, ngan, wah }) {
    let result = (Number.parseInt(rai) * 400) + (Number.parseInt(ngan) * 100) + Number.parseInt(wah);
    ...
    return (
      <div>
        <div className="ui form fluid segment input-box">
          <label id="input-text"><h3>Please select input land measurement system</h3></label>
          <InputSystemSelector handleInputChange={this.handleSystemSelect}/>
          <SystemInput system={this.state.selectedInputSystem} handleSubmit={this.handleSubmit}/>
        </div>
        <div className="ui form fluid segment output-box">
          <label id="input-text"><h3>Please select target land measurement system</h3></label>
          <OutputSystemSelector handleOutputChange={this.handleOutputSystemSelect}/>
          {this.state.results.amount ? <ResultDisplay results={this.state.results}/> : null }
        </div>
      </div>
    );
  }
